...
export function CommandPalette() {
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogContent className="brutalist-border max-w-2xl p-0 gap-0 overflow-hidden">
        <div className="flex items-center gap-3 p-4 border-b-2 border-border bg-gradient-to-r from-card to-primary/5">
          ...
          <Input
            ...
            className="border-0 focus-visible:ring-0 font-mono text-base"
            autoFocus
          />
          <Badge variant="outline" className="border-2 border-black font-black text-xs">
            ⌘K
          </Badge>
        </div>
                      ...
                      {cmds.map((cmd) => {
                        ...
                        return (
                          <button
                            key={cmd.id}
                            onClick={cmd.action}
                            className="w-full flex items-center gap-3 p-3 hover:bg-primary/10 border-2 border-transparent hover:border-primary transition-all group"
                          >
                            <div className="w-9 h-9 bg-primary/10 flex items-center justify-center border-2 border-black group-hover:bg-primary group-hover:scale-110 transition-all">
                              <Icon className="w-4 h-4 group-hover:text-primary-foreground" />
                            </div>
                            <div className="flex-1 text-left min-w-0">
                              <div className="font-black uppercase text-sm truncate">{cmd.label}</div>
                              <div className="text-xs font-mono text-muted-foreground truncate">
                                {cmd.description}
                              </div>
                            </div>
                            {cmd.shortcut && (
                              <Badge variant="outline" className="border-2 border-black font-black text-xs shrink-0">
                                {cmd.shortcut}
                              </Badge>
                            )}
